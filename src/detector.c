...
void validate_detector_precision_recall_map(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, network *existing_net)
{
    int j,letter_box;
    list *options = read_data_cfg(datacfg);
    char *valid_images = option_find_str(options, "test", "data/train.txt");
...
void run_detector(int argc, char **argv)
{
    ...
    else if (0 == strcmp(argv[2], "precision")) validate_detector_precision_recall_map(datacfg, cfg, weights, 0.01, 0.01, map_points,NULL);
